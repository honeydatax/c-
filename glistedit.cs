...
public void drawtext(Graphics canvas){
   ...
   public class Form1 : Form
		 
    {
		...
		public glistedit listedit1 = new glistedit(12);
        ...
        public Form1()
        {
			...
			listedit1.size=12;
			for(i=0;i<64;i++){
				listedit1.text=listedit1.text+" item:" +Convert.ToString(i)+"\n";
		...
		void KeyPresss(object sender,KeyPressEventArgs e){
			selected=listedit1.check(e);
			if(selected>-1){
				this.Refresh();
			}
		}
		void Keyit(object sender,KeyEventArgs e){
			selected=listedit1.keys(e);
			if(selected>-1){
				this.Refresh();
			}
		}
        ...
        void draw(){
			string s="";
			listedit1.drawtext(canvas);
			if (selected>-1)s=listedit1.caption[selected];
