...
namespace FormWithButton
{
		public class gbutton{
			...
			public void draw(Graphics canvas){
				label1.drawLabel(x,y,w,h,size,caption,raised,canvas);
			}
			...
			public bool up(MouseEventArgs e){
				bool i=false;
				
			    if(e.Button==System.Windows.Forms.MouseButtons.Left && !raised){
		...
		public class glabel{
			public void drawLabel(int x,int y,int w,int h,int size,string s,bool raised,Graphics canvas){
			float xn;
			float yn;
			string sss=s;
			Font ff;
			...
			canvas.FillRectangle(Brushes.DarkGray,x,y,w,h);
			canvas.DrawString(sss,ff,Brushes.Black,xn,yn,ss);			
			if (raised){
				canvas.DrawLine(Pens.White,x,y,w+x,y);
				canvas.DrawLine(Pens.White,x,y,x,y+h);
				canvas.DrawLine(Pens.Black,x+w,y,w+x,y+h);
				canvas.DrawLine(Pens.Black,x,y+h,x+w,y+h);

			}else{
				canvas.DrawLine(Pens.Black,x,y,w+x,y);
				canvas.DrawLine(Pens.Black,x,y,x,y+h);
				canvas.DrawLine(Pens.White,x+w,y,w+x,y+h);
				canvas.DrawLine(Pens.White,x,y+h,x+w,y+h);
